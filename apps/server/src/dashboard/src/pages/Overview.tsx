...
export function Overview() {
  ...
  return (
    <div className="space-y-8">
      {/* Hero / Status Banner */}
      <div className={`card p-6 relative overflow-hidden ${allGood ? 'border-emerald-500/20' : 'border-red-500/20'}`}>
        <div className="absolute inset-0 bg-gradient-to-r from-brand-500/5 via-transparent to-brand-400/5" />
      ...
      {/* Stats — plain English labels */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard
          label="Services Online"
          hint="How many of your sites/APIs are responding"
          value={healthyCount}
          color="text-emerald-400"
        />
        <StatCard
          label="Services Offline"
          hint="Sites or APIs that aren't responding"
          value={downCount}
          color={downCount > 0 ? 'text-red-400' : 'text-gray-500'}
        />
        <StatCard
          label="Open Issues"
          hint="Problems that haven't been fixed yet"
          value={incidentList.length}
          color={incidentList.length > 0 ? 'text-red-400' : 'text-gray-500'}
        />
        <StatCard
          label="Recent Activity"
          hint="Things that happened recently"
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Problems ScanWarp detected that need your attention</p>
            </div>
          ...
          <div className="card divide-y divide-[#1e2333]">
            {incidentList.length === 0 ? (
              <div className="p-5 text-center">
                <p className="text-gray-500 text-sm">No open issues — you're in the clear!</p>
              </div>
            ) : (
              incidentList.slice(0, 5).map((inc) => (
                <Link
                  key={inc.id}
                  to={`/incidents/${inc.id}`}
                  className="flex items-center justify-between p-4 hover:bg-surface-overlay/50 transition-colors"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <Badge label={inc.severity} />
                    <span className="text-sm truncate">
                      {inc.diagnosis_text?.slice(0, 80) || `Issue with ${inc.events.length} related event${inc.events.length > 1 ? 's' : ''}`}
                    </span>
                  </div>
                  <span className="text-xs text-gray-500 shrink-0 ml-3">{timeAgo(inc.created_at)}</span>
                </Link>
              ))
            )}
          </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Sites and APIs that ScanWarp is checking on</p>
            </div>
          ...
          <div className="card divide-y divide-[#1e2333]">
            {monitorList.length === 0 ? (
              <div className="p-5 text-center">
                <p className="text-gray-500 text-sm">No health checks set up yet.</p>
                <p className="text-xs text-gray-600 mt-1">Run <code className="text-brand-400 bg-brand-500/10 px-1.5 py-0.5 rounded text-xs">scanwarp init</code> to get started.</p>
              </div>
            ) : (
              monitorList.map((m) => (
                <Link
                    ...
                    <span
                      className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                        m.status === 'up'
                          ...
                          : m.status === 'down'
                          ? 'bg-red-400 shadow-sm shadow-red-400/50 animate-scan-pulse'
                          : 'bg-gray-500'
                      }`}
                      title={m.status === 'up' ? 'Responding normally' : m.status === 'down' ? 'Not responding' : 'Haven\'t checked yet'}
                    />
                    <span className="text-sm truncate">{m.url}</span>
                  </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">High-priority errors ScanWarp caught recently</p>
            </div>
              ...
              <div key={e.id} className="p-4 flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    from <span className="text-gray-400">{friendlySource(e.source)}</span> · {timeAgo(e.created_at)}
                  </p>
                </div>
              </div>
            ))}
